// Including the necessary libs
#include <iostream>

// using the std namespace to make life easier
using namespace std;

// define uint8_t type 
typedef unsigned char uint8_t; 



/*

Defining Variables

*/



/* 
AES key length in 32-bit words (for AES-128).
Aes-128 = 4 words (4*32 = 128 bits)
Aes-192 = 6 words (6*32 = 192 bits)
Aes-256 = 8 words (8*32 = 256 bits)
*/
static uint8_t Nk;

/* 
Number of rounds in AES (for AES-128).
Aes-128 = 10 rounds
Aes-192 = 12 rounds
Aes-256 = 14 rounds
*/
static uint8_t Nr; 

static uint8_t state[4][4] = {0x00, 0x00, 0x00, 0x00},
                             {0x00, 0x00, 0x00, 0x00},
                             {0x00, 0x00, 0x00, 0x00},
                             {0x00, 0x00, 0x00, 0x00}; // Define the state / block

static uint8_t round_keys[240]; // round key has max 240 bytes (in case of aes-256 15 rounds * 16 bytes)

// define the substitution box we later use as a lookup table
static constexpr uint8_t sbox[256] = {
   
};

static constexpr uint8_t inverse_sbox[256] = {
   
};

// array with the rcon
static constexpr uint8_t rotate_constants[11] = {
  0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};



/*

These gmul tables have been precomputed to prevent dynamic memory allocation which is better for embedded systems
apart from that is on runtime calculation of those values annoying and resource consuming apart from some other performance reasons

*/

static constexpr uint8_t gmul2[256] = {
 
};



static constexpr uint8_t gmul3[256] = {
 
};



static constexpr uint8_t gmul9[256] = {
 
};



static constexpr uint8_t gmul11[256] = {
 
};



static constexpr uint8_t gmul13[256] = {
 
};



static constexpr uint8_t gmul14[256] = {
 
};


/*

Defining Functions/classes

*/


// multiplying in the galois field
uint8_t galois_multiply(uint8_t a, uint8_t b) {
  uint8_t p = 0; // accumulated product
  bool hi_bit_set;

  for (int i = 0; i < 8; i++) {
      if (b & 1) p ^= a; // if the lowest bit of b is set, XOR a into p
      hi_bit_set = (a & 0x80); // check if the highest bit of a is set
      a <<= 1; // multiply a by 2 (with left bitshift)
      if (hi_bit_set) a ^= 0x1b; // if the highest bit was set, reduce modulo the AES polynomial
      b >>= 1; // divide b by 2
  }

  return p;
}

// A function that substitutes all bytes in the state with their bytes in the sbox
void SubBytes() {
  state[0][0] = sbox[state[0][0]]; 
  state[0][1] = sbox[state[0][1]]; 
  state[0][2] = sbox[state[0][2]]; 
  state[0][3] = sbox[state[0][3]];

  state[1][0] = sbox[state[1][0]]; 
  state[1][1] = sbox[state[1][1]]; 
  state[1][2] = sbox[state[1][2]]; 
  state[1][3] = sbox[state[1][3]];

  state[2][0] = sbox[state[2][0]]; 
  state[2][1] = sbox[state[2][1]]; 
  state[2][2] = sbox[state[2][2]]; 
  state[2][3] = sbox[state[2][3]];

  state[3][0] = sbox[state[3][0]]; 
  state[3][1] = sbox[state[3][1]]; 
  state[3][2] = sbox[state[3][2]]; 
  state[3][3] = sbox[state[3][3]];
    /* This code is the same as :
  for(int row = 0; row < 4; row++) {
    state[row][0] = inverse_sbox[state[row][0]]; 
    state[row][1] = inverse_sbox[state[row][1]]; 
    state[row][2] = inverse_sbox[state[row][2]]; 
    state[row][3] = inverse_sbox[state[row][3]];
  }
  or 

  for(int row = 0; row < 4; row++) {
    for(int col = 0; col < 4; col++) {
      state[row][col] = 
    }
  }
  
  Maybe optimizing this was overkill ...
  */
  return;
}


// Same as SubBytes but with the inverse substitution lookup table
void InvSubBytes() {

  state[0][0] = inverse_sbox[state[0][0]]; 
  state[0][1] = inverse_sbox[state[0][1]]; 
  state[0][2] = inverse_sbox[state[0][2]]; 
  state[0][3] = inverse_sbox[state[0][3]];

  state[1][0] = inverse_sbox[state[1][0]]; 
  state[1][1] = inverse_sbox[state[1][1]]; 
  state[1][2] = inverse_sbox[state[1][2]]; 
  state[1][3] = inverse_sbox[state[1][3]];

  state[2][0] = inverse_sbox[state[2][0]]; 
  state[2][1] = inverse_sbox[state[2][1]]; 
  state[2][2] = inverse_sbox[state[2][2]]; 
  state[2][3] = inverse_sbox[state[2][3]];

  state[3][0] = inverse_sbox[state[3][0]]; 
  state[3][1] = inverse_sbox[state[3][1]]; 
  state[3][2] = inverse_sbox[state[3][2]]; 
  state[3][3] = inverse_sbox[state[3][3]];

  return;
}

// Shifting the rows of the state
void ShiftRows() {
  // Temporary storage for byte shifting
  uint8_t temp[2];

  // Note: parallel assignment without temp storage looks more efficient but isn't weirdly enough
  
  // row 2: 1-byte left shift
  temp[0] = state[1][0];
  state[1][0] = state[1][1];
  state[1][1] = state[1][2];
  state[1][2] = state[1][3];
  state[1][3] = temp[0];

  // row 3: 2-byte left shift
  temp[0] = state[2][0], temp[1] = state[2][1];
  state[2][0] = state[2][2];
  state[2][1] = state[2][3];
  state[2][2] = temp[0];
  state[2][3] = temp[1];

  // row 4: 3-byte left shift
  temp[0] = state[3][3];
  state[3][3] = state[3][2];
  state[3][2] = state[3][1];
  state[3][1] = state[3][0];
  state[3][0] = temp[0];

  return;
  
}

// Reverse shifting the rows of the state
void InvShiftRows() {
    // Temporary storage for byte shifting
    uint8_t temp[2];

    // row 2: 1-byte right shift
    temp[0] = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp[0];

    // row 3: 2-byte right shift
    temp[0] = state[2][3], temp[1] = state[2][2];
    state[2][3] = state[2][1];
    state[2][2] = state[2][0];
    state[2][1] = temp[0];
    state[2][0] = temp[1];

    // row 4: 3-byte right shift
    temp[0] = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = temp[0];
    
    return;
}


// A function used to do the mixcolumns operation on the state
void MixColumns() {
  uint8_t temp[4];

  for (uint8_t col = 0; col < 4; col++) {
      // perform matrix multiplication for each column using precomputed gmul tables
      temp[0] = gmul2[state[0][col]] ^ gmul3[state[1][col]] ^ state[2][col] ^ state[3][col];
      temp[1] = state[0][col] ^ gmul2[state[1][col]] ^ gmul3[state[2][col]] ^ state[3][col];
      temp[2] = state[0][col] ^ state[1][col] ^ gmul2[state[2][col]] ^ gmul3[state[3][col]];
      temp[3] = gmul3[state[0][col]] ^ state[1][col] ^ state[2][col] ^ gmul2[state[3][col]];

      // write the result back to the block
      state[0][col] = temp[0];
      state[1][col] = temp[1];
      state[2][col] = temp[2];
      state[3][col] = temp[3];
  }

  return;
}

// A function used to do the inverse mixcolumns operation on the state
void InvMixColumns() {
  uint8_t temp[4];

  for (uint8_t col = 0; col < 4; col++) {
      temp[0] = gmul14[state[0][col]] ^ gmul11[state[1][col]] ^ gmul13[state[2][col]] ^ gmul9[state[3][col]];
      temp[1] = gmul9[state[0][col]] ^ gmul14[state[1][col]] ^ gmul11[state[2][col]] ^ gmul13[state[3][col]];
      temp[2] = gmul13[state[0][col]] ^ gmul9[state[1][col]] ^ gmul14[state[2][col]] ^ gmul11[state[3][col]];
      temp[3] = gmul11[state[0][col]] ^ gmul13[state[1][col]] ^ gmul9[state[2][col]] ^ gmul14[state[3][col]];

      // write the result back to the state
      state[0][col] = temp[0];
      state[1][col] = temp[1];
      state[2][col] = temp[2];
      state[3][col] = temp[3];
  }
  return;
}


// function to xor round key with block (column wise)
void AddRoundKey(int round) {    

  short offset = round * 16;
  for (uint8_t row = 0; row < 4; row++) {
    short x = row * 4;
    state[0][row] ^= round_keys[(offset) + (x) + 0];
    state[1][row] ^= round_keys[(offset) + (x) + 1];
    state[2][row] ^= round_keys[(offset) + (x) + 2];
    state[3][row] ^= round_keys[(offset) + (x) + 3];
  }
  return;
}


// The key expansion function for AES-(128/192/256)
void KeyExpand(uint8_t key[]) {
    unsigned int i,j;
    uint8_t tempa[4];

    // Step 1: Copy the original key to the start of the output array.
    for (i = 0; i < Nk * 4; ++i) {
        round_keys[i] = key[i];
    }

    // Step 2: Expand the key.
    for (i = Nk; i < 4 * (Nr + 1); ++i) {
        j = (i - 1) * 4;
        tempa[0] = round_keys[j + 0];
        tempa[1] = round_keys[j + 1];
        tempa[2] = round_keys[j + 2];
        tempa[3] = round_keys[j + 3];

        // Apply the key expansion schedule for words multiple of Nk
        if (i % Nk == 0) {
            uint8_t temp = tempa[0];
            tempa[0] = sbox[tempa[1]] ^ rotate_constants[i / Nk];  // Apply Sbox and Rcon
            tempa[1] = sbox[tempa[2]];
            tempa[2] = sbox[tempa[3]];
            tempa[3] = sbox[temp];
        }

        // If Nk = 8, and i is a multiple of 8, we also apply a special transformation (2nd round of S-box)
        if (Nk == 8 && i % Nk == 4) {
            tempa[0] = sbox[tempa[0]];
            tempa[1] = sbox[tempa[1]];
            tempa[2] = sbox[tempa[2]];
            tempa[3] = sbox[tempa[3]];
        }

        // Step 3: XOR with the word from Nk positions earlier.
        round_keys[i * 4 + 0] = round_keys[(i - Nk) * 4 + 0] ^ tempa[0];
        round_keys[i * 4 + 1] = round_keys[(i - Nk) * 4 + 1] ^ tempa[1];
        round_keys[i * 4 + 2] = round_keys[(i - Nk) * 4 + 2] ^ tempa[2];
        round_keys[i * 4 + 3] = round_keys[(i - Nk) * 4 + 3] ^ tempa[3];
    }
}

// the main logic that encrypts a 16 byte 4x4 block 
void AES_Encrypt() {
    short round = 1;
    // xor the first initial round key with the block (round key 0) 
    AddRoundKey(0); 
    // Main rounds (excluding the final) 

    for (; round <= (Nr - 1); round++) {
        // substitute bytes
        SubBytes();
        // shift rows
        ShiftRows();
        // mix columns
        MixColumns();

        // and finally add the round key
        AddRoundKey(round);

    }

    // final round adding round key 10 (no MixColumns)
    SubBytes();
    ShiftRows();
    AddRoundKey(round);
}



void AES_Decrypt() {
    short round = Nr;
    // XOR the last round key with the block
    AddRoundKey(round); // No inverse function because xor a ^ b = c, c ^ b = a, c ^ a = b
    InvShiftRows();
    InvSubBytes();

    // 9 Main rounds (excluding the final)
    for (round -= 1; round >= 1; round--) {

        AddRoundKey(round);
        InvMixColumns();
        InvShiftRows();
        InvSubBytes();

    }

    AddRoundKey(0); // add last round key in decrypting process 
}






int main() {
    // Aes-128
    
    Nk = 4; // 4 32-bit words (for aes-128)
    Nr = 10; // rounds

    state[0][0] = 0x80; // set plaintext to 80000000000000000000000000000000
    uint8_t key[16] = { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};//,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 };
    KeyExpand(key);
    AES_Encrypt();
    AES_Decrypt();
    
    return 0;
}



// THIS IS A TEST VECTOR FROM THE OFFICIAL NIST (AESVS VarTxt test data for ECB / AES Known Answer Test (KAT) Vectors)
// https://csrc.nist.gov/Projects/Cryptographic-Algorithm-Validation-Program/Block-Ciphers
//KEY = 00000000000000000000000000000000
//PLAINTEXT = 80000000000000000000000000000000
//CIPHERTEXT = 3ad78e726c1ec02b7ebfe92b23d9ec34 voor 128-bits en ddc6bf790c15760d8d9aeb6f9a75fd4e voor 256-bits
